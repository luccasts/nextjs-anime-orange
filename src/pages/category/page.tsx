...
export default function CategoryPage({ data, error, isLoading }: any) {
  ...
  const categoryID = data?.map((d: any) =>
    d.details.category.map((c: any) => {
      if (c.name == routerID) {
        return c.id;
      }
    })
  ...
  const withoutUndefined = categoryID
    ?.find((c: string) => c !== undefined)
    ?.filter((c: string) => c !== undefined);

  const categoryName = withoutUndefined
    ? handleCategoryName(withoutUndefined[0])
      ...
      <WrapperMain>
        <WrapperSection>
          ...
          <ul className={styles.list}>
            {data?.map((d: any) => (
              <PosterAnime key={d.id} apiAnime={d} />
            ))}
          </ul>
        </WrapperSection>
      </WrapperMain>
